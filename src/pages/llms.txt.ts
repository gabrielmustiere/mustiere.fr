...
export const GET: APIRoute = async () => {
  ...
  lines.push(`- Rôle : ${SITE.author.jobTitle}`);
  ...
  lines.push(
    `- [CV PDF](${SITE.url}/cv-gabriel-mustiere.pdf) : CV détaillé, dernière mise à jour avril 2026.`
  );
  ...
  lines.push('## Optional');
  lines.push('');
  lines.push(`- [llms-full.txt](${SITE.url}/llms-full.txt) : contenu intégral du site en markdown pour les LLMs.`);
  lines.push(`- [Sitemap](${SITE.url}/sitemap-index.xml)`);
  lines.push(`- [robots.txt](${SITE.url}/robots.txt) : politique d'accès des crawlers (IA inclus).`);
  lines.push('');
